...
import { X, UserPlus, Upload, FileText, Printer, Download, CheckCircle2 } from 'lucide-react';
import type { Patient } from '../../datapoints/patients';
import { CENTER_INITIALS } from '../../datapoints/scheduling';
import {
    ...
    <div>
      ...
      <div className={`flex rounded-lg border overflow-hidden focus-within:ring-2 focus-within:ring-primary/20 transition-all ${error ? 'border-destructive' : 'border-border focus-within:border-primary'}`}>
        <select
          ...
          className="shrink-0 bg-muted text-foreground text-sm px-2 py-2 border-r border-border focus:outline-none cursor-pointer"
          style={{ minWidth: '80px' }}
        >
          {COUNTRY_CODES.map(c => (
        ...
        <input
          ...
          className="flex-1 text-sm bg-input-background text-foreground px-3 py-2 focus:outline-none"
        />
      </div>
      {onWhatsapp !== undefined && (
        <label className="flex items-center gap-2 mt-1.5 cursor-pointer select-none">
          <input type="checkbox" checked={whatsapp ?? false} onChange={e => onWhatsapp(e.target.checked)} className="w-3.5 h-3.5 accent-primary rounded" />
          <span className="text-xs text-muted-foreground">WhatsApp available on this number</span>
        </label>
      )}
...
export function PatientRegistrationOverlay({ selectedCenter, patients, onRegister, onClose }: Props) {
  ...
  if (receiptData) {
    ...
    return (
      <Modal onClose={closeReceipt} className="max-w-4xl h-[90vh]" overlayClassName="z-[60]">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <p className="text-base font-semibold text-foreground font-mono">{receiptData.receiptNumber}</p>
            <p className="text-xs text-muted-foreground mt-0.5">{receiptData.patient.name} · {selectedCenter}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <IconButton icon={<X size={18} />} label="Close" onClick={closeReceipt} className="ml-1" />
          </div>
        </div>
        {/* Receipt preview */}
        <iframe
    ...
    return (
      <Modal onClose={onClose} size="md" overlayClassName="z-[60]">
        <div className="p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Patient Registered Successfully</h2>
          <p className="text-sm font-semibold text-foreground mb-1">{successInfo.name}</p>
          <p className="text-sm font-mono text-muted-foreground mb-1">{successInfo.mrn}</p>
          <p className="text-sm text-muted-foreground mb-8">{selectedCenter}</p>
          <Button onClick={onClose} size="sm" className="w-full rounded-xl py-2.5">
            Done
          </Button>
        </div>
      </Modal>
  ...
  return (
    <Modal onClose={onClose} className="max-w-2xl" overlayClassName="z-[60]">
      ...
      <ModalBody className="py-6 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField label="MRN / UHID" hint="(Auto-generated)">
                  <TextField value={mrn} disabled className="font-mono bg-muted/50 cursor-not-allowed opacity-70" />
                </FormField>
                ...
                <div className="flex items-center gap-6">
                  {(['M', 'F', 'Other'] as const).map(g => (
                    <label key={g} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="gender" value={g} checked={form.gender === g} onChange={() => setField('gender', g)} className="w-4 h-4 accent-primary" />
                      <span className="text-sm text-foreground">{g === 'M' ? 'Male' : g === 'F' ? 'Female' : 'Other'}</span>
                    </label>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField label="Age">
                  <TextField value={age} disabled placeholder="Auto-calculated" className="bg-muted/50 cursor-not-allowed opacity-70" />
                </FormField>
              </div>

              <FormField className="max-w-xs" label="Registration Date" hint="(up to 7 days back)">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-foreground mb-1">
                          {selectedIdentifier?.label} Number <span className="text-destructive">*</span>
                        </label>
                        ...
                        {errors.identifierNumber && <p className="text-xs text-destructive mt-1">{errors.identifierNumber}</p>}
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-foreground mb-1">Upload Document</label>
                        <input type="file" ref={fileInputRef} accept=".pdf,.jpg,.jpeg,.png" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) setIdentifierDoc({ name: f.name }); e.target.value = ''; }} />
                        {identifierDoc ? (
                          <div className="flex items-center gap-2 px-3 py-2 bg-primary/5 border border-primary/20 rounded-lg">
                            <FileText size={13} className="text-primary shrink-0" />
                            <span className="text-xs text-foreground flex-1 truncate">{identifierDoc.name}</span>
                            <button type="button" onClick={() => setIdentifierDoc(null)} className="text-muted-foreground hover:text-destructive transition-colors"><X size={13} /></button>
                          </div>
                        ) : (
                          <button type="button" onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 w-full px-3 py-2 border border-dashed border-border rounded-lg text-sm text-muted-foreground hover:border-primary hover:text-primary transition-colors">
                            <Upload size={13} />
          ...
          <section onFocus={() => handleSectionFocus(2)}>
            <SectionHeading>
              Address Details
              {isInternational && <span className="text-primary font-normal normal-case ml-2">· International</span>}
            </SectionHeading>
            <div className="space-y-4">
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormField label="Country">
                      <TextField value="India" disabled className="bg-muted/50 cursor-not-allowed opacity-70" />
                    </FormField>
                  </div>
                </>
              )}
          ...
          <section onFocus={() => handleSectionFocus(5)}>
            ...
            <div className="space-y-4">
              ...
              {form.payorType === 'insurance' && (
                <div className="bg-muted/30 border border-border rounded-xl p-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField className="md:col-span-2" label="Insurance / TPA Name" required error={errors.insName}>
              ...
              {form.payorType === 'corporate' && (
                <div className="bg-muted/30 border border-border rounded-xl p-4 space-y-5">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Insurance / TPA Details</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Policy Details</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              ...
              {form.referralSource === 'referral' && (
                <div className="bg-muted/30 border border-border rounded-xl p-4 space-y-4">
                  <p className="text-xs text-muted-foreground">At least one of the following fields is required.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {errors.referralAtLeastOne && <p className="text-xs text-destructive">{errors.referralAtLeastOne}</p>}
                </div>
              )}
            </div>
          </section>
          ...
          <section onFocus={() => handleSectionFocus(8)}>
            ...
            <div className="space-y-4">
              <div className="flex items-end gap-3">
                <FormField className="flex-1" label="Document Type">
                ...
                <div className="shrink-0">
                  <input type="file" ref={docFileRef} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" className="hidden" onChange={handleDocUpload} />
                  <button
                    type="button"
                    disabled={!docType}
                    onClick={() => docFileRef.current?.click()}
                    className="flex items-center gap-2 px-4 py-2 text-sm font-medium border border-border rounded-lg hover:bg-muted disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  >
                    <Upload size={14} />
                    Upload File
                  </button>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-muted/50">
                        <th className="text-left py-2 px-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Type</th>
                        <th className="text-left py-2 px-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">File Name</th>
                    ...
                    <tbody>
                      {uploadedDocs.map(doc => (
                        <tr key={doc.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                          ...
                          <td className="py-2 px-3 text-sm text-foreground max-w-[160px] truncate">{doc.fileName}</td>
                          <td className="py-2 px-3 text-sm text-muted-foreground whitespace-nowrap">{doc.date}</td>
                          <td className="py-2 px-3 text-right">
                            <button type="button" onClick={() => setUploadedDocs(prev => prev.filter(d => d.id !== doc.id))} className="text-muted-foreground hover:text-destructive transition-colors">
                              <X size={14} />
                            </button>
                          </td>
                        </tr>
          ...
          <section onFocus={() => handleSectionFocus(9)}>
            ...
            <div className={`border rounded-xl p-4 transition-colors ${errors.dataConsent ? 'border-destructive bg-destructive/5' : 'border-border bg-muted/30'}`}>
              ...
              {errors.dataConsent && <p className="text-xs text-destructive mt-2 ml-7">{errors.dataConsent}</p>}
            </div>
          </section>
      ...
      <ModalFooter>
        ...
        <Button size="sm" onClick={handleSubmit} className="gap-2 px-5">
          <UserPlus size={15} />
          Register Patient
        </Button>
      </ModalFooter>

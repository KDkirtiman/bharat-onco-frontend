...
import type { Appointment, AppointmentType } from '../../datapoints/scheduling';
import {
...
import { SearchCombobox } from '../../components/controls/SearchCombobox';
          ...
          summary={
            <>
              <p className="text-sm font-semibold text-foreground">
                ...
                <span className="font-normal text-muted-foreground"> · {selectedPatient?.mrn}</span>
              </p>
              <p className="text-sm text-foreground">
                {APPOINTMENT_TYPE_LABELS[confirmedAppt.type]}
                {confirmedDoctor && (
                  <span className="text-muted-foreground"> · {confirmedDoctor.name}</span>
                )}
              </p>
              <p className="text-sm text-foreground">
                {fmtDate(confirmedAppt.date)} · {fmtTime(confirmedAppt.time)}
              </p>
              <p className="text-xs font-mono text-muted-foreground pt-1">{confirmedAppt.visitId}</p>
            </>
          }
        />
      </Modal>
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <ModalBody className="space-y-4">
        {!selectedPatient ? (
          <FormField
            label={<>Patient <span className="text-xs text-muted-foreground font-normal ml-2">{selectedCenter} center</span></>}
            required
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-primary/20 bg-primary/5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{selectedPatient.mrn} · {selectedPatient.phone}</p>
            </div>
            <IconButton
              icon={<X size={14} />}
              label="Clear patient"
      ...
      <ModalFooter>
        ...
        <Button size="sm" onClick={handleConfirm} className="gap-2 px-5">
